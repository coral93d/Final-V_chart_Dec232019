...
namespace AspNetRoleBasedSecurity.Controllers
{
    public class PostController : Controller
    {
        ...
        public ActionResult AddPost(PostModel Post)
        {
            try
            {
                if (Post.Tags != null)
                {
                    ...
                    {
                       // var loggg = System.Web.HttpContext.Current.User.Identity.GetUserName();
                        PostRepository PostRepo = new PostRepository();
                       // PostRepository Prof = new PostRepository();
                        //var comments = Model.Comments.Where(Modelitems => (Modelitems.ArticleId.Equals(item.Id)) && (Modelitems.Com_mark) == true).ToList();
                      //  var ProfileInfo = Prof.Users().Where(Modelitems => (Modelitems.User_name.Equals(loggg)) && (Modelitems.Process) != null).ToList();
                        //var ProfileInfo = Prof.Users().Where((Users => Users.us) && (Users => Users.AllProcess != null ));
        ...
        public ActionResult GetAllPost(string Searchby, string search)
        {
            ...
            var Flash_msg = PostRepo.Flash_News();
            ...
            return View(new PostDetails { Post = post, Comments = comments, popular = popular, Track = Tags, tagfilter = tagfilter, Filter = filter, Falsh = Flash_msg });
        }

        [HttpPost]
        public ActionResult GetAllPost(string Comments)
        {
            try
            {
                ...
                com.Parameters.AddWithValue("@Comment", Comments);
        ...
        public ActionResult GetAllPostUser(string Searchby, string search)
        {
            ...
            PostRepository PostRepo3 = new PostRepository();
            PostRepository PostRepo4 = new PostRepository();
            PostRepository getfalsh = new PostRepository();
        ...
        public ActionResult Users(string search)
        {
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                users = (PostRepo.Users().Where(User => User.user.StartsWith(search, StringComparison.InvariantCultureIgnoreCase) || search == null).ToList());
        ...
        public ActionResult Tags(string search)
        {
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                track = (PostRepo.Tags().Where(User => User.Track_Name.StartsWith(search, StringComparison.InvariantCultureIgnoreCase) || search == null).ToList());
